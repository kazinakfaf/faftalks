/** 
 * Forked and modified https://github.com/fafafaf/fafafaf.github.io
*/
...
function ReplayParser(data) {
	...
	function readString() {
		...
		return utfDecoder.decode(new Uint8Array(tmp))
	}
			...
			switch(message_op) {
				...
				case ECmdStreamOp.CMDST_LuaSimCallback:
					...
					if (luaName == "GiveResourcesToPlayer")
						if ("Msg" in lua)
							if (lua["From"] -1 != -2  &&  lua["Msg"]["to"] != 'notify' ) {
								var message = {
									player_id: army[lua["From"]-1]["OwnerID"],
								...
								if (army[lua["From"]-1] && army[lua["From"]-1]["PlayerName"] == lua["Sender"] )  {
									...
									message.to = army[lua["Msg"]["to"] - 1]["PlayerName"];
									chat.push(message);
